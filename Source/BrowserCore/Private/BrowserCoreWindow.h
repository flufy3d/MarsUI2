 ...
 */
struct FBrowserCoreWindowInfo
{
	FBrowserCoreWindowInfo(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FBrowserCoreHandler> InHandler) 
		: Browser(InBrowser)
		, Handler(InHandler)
	{}
	CefRefPtr<CefBrowser> Browser;
	CefRefPtr<FBrowserCoreHandler> Handler;
};
...
class FBrowserCoreWindow
	: public IBrowserCoreWindow
	, public TSharedFromThis<FBrowserCoreWindow>
{
	// Allow the Handler to access functions only it needs
	friend class FBrowserCoreHandler;

	// The BrowserCoreSingleton should be the only one creating instances of this class
	friend class FBrowserCoreSingleton;
...
private:
	 ...
	 */
	FBrowserCoreWindow(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FBrowserCoreHandler> InHandler, FString InUrl, TOptional<FString> InContentsToLoad, bool ShowErrorMessage, bool bThumbMouseButtonNavigation, bool bUseTransparency);

public:
	/** Virtual Destructor. */
	virtual ~FBrowserCoreWindow();
...
public:
	...
	virtual EBrowserCoreDocumentState GetDocumentLoadingState() const override;
	...
	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnDocumentStateChanged, FOnDocumentStateChanged);
	virtual FOnDocumentStateChanged& OnDocumentStateChanged() override
	{
		return DocumentStateChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnTitleChanged, FOnTitleChanged);
	virtual FOnTitleChanged& OnTitleChanged() override
	{
		return TitleChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnUrlChanged, FOnUrlChanged);
	virtual FOnUrlChanged& OnUrlChanged() override
	{
		return UrlChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnToolTip, FOnToolTip);
	virtual FOnToolTip& OnToolTip() override
	{
		return ToolTipEvent;
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnNeedsRedraw, FOnNeedsRedraw);
	...
	{
		return BrowserCoreHandler->OnCreateWindow();
	}
	...
	{
		return BrowserCoreHandler->OnBeforePopup();
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnShowPopup, FOnShowPopup);
	virtual FOnShowPopup& OnShowPopup() override
	{
		return ShowPopupEvent;
	}

	DECLARE_DERIVED_EVENT(FBrowserCoreWindow, IBrowserCoreWindow::FOnDismissPopup, FOnDismissPopup);
...
private:
	 ...
	 */
	bool RequestCreateWindow(const TSharedRef<IBrowserCoreWindow>& NewBrowserWindow, const TSharedPtr<IBrowserCorePopupFeatures>& BrowserPopupFeatures);
	
	//bool SupportsCloseWindows();
	//bool RequestCloseWindow(const TSharedRef<IBrowserCoreWindow>& BrowserWindow);
...
public:
	...
	/**
	 * Called from the BrowserCoreSingleton tick event. Should test wether the widget got a tick from Slate last frame and set the state to hidden if not.
	 */
...
private:
	...
	EBrowserCoreDocumentState DocumentState;
	...
	CefRefPtr<FBrowserCoreHandler> BrowserCoreHandler;
