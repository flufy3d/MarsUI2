 ...
 */
class FBrowserCoreByteResource
	: public CefResourceHandler
{
public:
	/**
	 */
	FBrowserCoreByteResource(const void* InBuffer, int32 InSize) : Position(0), Size(InSize)
	{
		Buffer = new unsigned char[Size];
		FMemory::Memcpy(Buffer, InBuffer, Size);
	}
	
	~FBrowserCoreByteResource()
	{
		delete[] Buffer;
	}
...
private:
	...
	IMPLEMENT_REFCOUNTING(FBrowserCoreByteResource);
};
