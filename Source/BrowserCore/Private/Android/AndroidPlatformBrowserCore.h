...
class SAndroidBrowserCoreWidget;
class SBrowserCoreView;
	...
	, public TSharedFromThis<FBrowserCoreWindow>
{
	...
	// CreateWidget should only be called by the BrowserCoreView
	friend class SBrowserCoreView;
...
private:
	...
	*/
	FBrowserCoreWindow(FString InUrl, TOptional<FString> InContentsToLoad, bool ShowErrorMessage, bool bThumbMouseButtonNavigation, bool bUseTransparency);

	/**
	 * Create the SWidget for this BrowserCoreWindow
	 */
...
public:
	...
	/**
	* Called from the BrowserCoreSingleton tick event. Should test whether the widget got a tick from Slate last frame and set the state to hidden if not.
	*/
	//void CheckTickActivity();

private:

	TSharedPtr<SAndroidBrowserCoreWidget> BrowserWidget;
