...
namespace {
	...
	bool ReadJSFunctionProperty(TSharedPtr<FWebJSScripting> Scripting, UProperty* Property, UProperty* Outer, void* Data, int32 ArrayIndex, CefRefPtr<ContainerType> Container, KeyType Key )
	{
		...
		if ( StructProperty->Struct != FCoreJSFunction::StaticStruct())
		{
			return false;
		}
		...
		FCoreJSFunction CallbackObject(Scripting, CallbackID);
		return SetPropertyValue<UStructProperty, FCoreJSFunction>(Property, Outer, Data, ArrayIndex, CallbackObject);
	}
