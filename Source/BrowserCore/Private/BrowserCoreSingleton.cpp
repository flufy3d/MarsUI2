...
#include "IBrowserCoreCookieManager.h"
...
class FBrowserCoreWindowFactory
	: public IBrowserCoreWindowFactory
{
public:

	virtual ~FBrowserCoreWindowFactory()
	{ }

	virtual TSharedPtr<IBrowserCoreWindow> Create(
		TSharedPtr<FBrowserCoreWindow>& BrowserWindowParent,
		TSharedPtr<FBrowserCoreWindowInfo>& BrowserWindowInfo) override
	{
		return IBrowserCoreModule::Get().GetSingleton()->CreateBrowserWindow(
			BrowserWindowParent,
			BrowserWindowInfo);
	}
...
class FNoBrowserCoreWindowFactory
	: public IBrowserCoreWindowFactory
{
public:

	virtual ~FNoBrowserCoreWindowFactory()
	{ }
...
FBrowserCoreSingleton::FBrowserCoreSingleton()
#if WITH_CEF3
	: BrowserCoreWindowFactory(MakeShareable(new FBrowserCoreWindowFactory()))
#else
	: BrowserCoreWindowFactory(MakeShareable(new FNoBrowserCoreWindowFactory()))
#endif
	, bDevToolsShortcutEnabled(UE_BUILD_DEBUG)
{
#if WITH_CEF3
	// The FBrowserCoreSingleton must be initialized on the game thread
	...
	// BrowserCoreApp implements application-level callbacks.
	BrowserCoreApp = new FBrowserCoreApp;
	BrowserCoreApp->OnRenderProcessThreadCreated().BindRaw(this, &FBrowserCoreSingleton::HandleRenderProcessCreated);
	...
	{
		UE_LOG(LogBrowserCore, Error, TEXT("Chromium Resources information not found at: %s."), *ResourcesPath);
	}
	...
	{
		UE_LOG(LogBrowserCore, Error, TEXT("Chromium Locales information not found at: %s."), *LocalesPath);
	}
	...
	{
		UE_LOG(LogBrowserCore, Error, TEXT("UnrealCEFSubProcess.exe not found, check that this program has been built and is placed in: %s."), *SubProcessPath);
	}
	CefString(&Settings.browser_subprocess_path) = *SubProcessPath;

	// Initialize CEF.
	bool bSuccess = CefInitialize(MainArgs, Settings, BrowserCoreApp.get(), nullptr);
	check(bSuccess);

	// Set the thread name back to GameThread.
	SetCurrentThreadName(TCHAR_TO_ANSI( *(FName( NAME_GameThread ).GetPlainNameString()) ));

	DefaultCookieManager = FCefBrowserCoreCookieManagerFactory::Create(CefCookieManager::GetGlobalManager(nullptr));
#endif
}

#if WITH_CEF3
void FBrowserCoreSingleton::HandleRenderProcessCreated(CefRefPtr<CefListValue> ExtraInfo)
{
	for (int32 Index = WindowInterfaces.Num() - 1; Index >= 0; --Index)
	{
		TSharedPtr<FBrowserCoreWindow> BrowserWindow = WindowInterfaces[Index].Pin();
...
FBrowserCoreSingleton::~FBrowserCoreSingleton()
{
	...
	for (int32 Index = 0; Index < WindowInterfaces.Num(); ++Index)
	{
		...
		{
			// Call CloseBrowser directly on the Host object as FBrowserCoreWindow::CloseBrowser is delayed
			BrowserWindow->InternalCefBrowser->GetHost()->CloseBrowser(true);
		}
	}

	// Just in case, although we deallocate BrowserCoreApp right after this.
	BrowserCoreApp->OnRenderProcessThreadCreated().Unbind();
	// CefRefPtr takes care of delete
	BrowserCoreApp = nullptr;
	// Shut down CEF.
	CefShutdown();
...
TSharedRef<IBrowserCoreWindowFactory> FBrowserCoreSingleton::GetBrowserCoreWindowFactory() const
{
	return BrowserCoreWindowFactory;
}

TSharedPtr<IBrowserCoreWindow> FBrowserCoreSingleton::CreateBrowserWindow(
	TSharedPtr<FBrowserCoreWindow>& BrowserWindowParent,
	TSharedPtr<FBrowserCoreWindowInfo>& BrowserWindowInfo
	)
{
#if WITH_CEF3
	...
	TSharedPtr<FBrowserCoreWindow> NewBrowserWindow(new FBrowserCoreWindow(BrowserWindowInfo->Browser, BrowserWindowInfo->Handler, InitialURL, ContentsToLoad, bShowErrorMessage, bThumbMouseButtonNavigation, bUseTransparency));
...
TSharedPtr<IBrowserCoreWindow> FBrowserCoreSingleton::CreateBrowserWindow(const FCreateBrowserWindowSettings& WindowSettings)
{
#if WITH_CEF3
	...
	if (AllowCEF)
	{
		...
		// BrowserCoreHandler implements browser-level callbacks.
		CefRefPtr<FBrowserCoreHandler> NewHandler(new FBrowserCoreHandler(WindowSettings.bUseTransparency));
		...
		{
			// Create new window
			TSharedPtr<FBrowserCoreWindow> NewBrowserWindow = MakeShareable(new FBrowserCoreWindow(
				Browser, 
				NewHandler, 
...
bool FBrowserCoreSingleton::Tick(float DeltaTime)
{
#if WITH_CEF3
...
FString FBrowserCoreSingleton::GetCurrentLocaleCode()
{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void FBrowserCoreSingleton::DeleteBrowserCookies(FString URL, FString CookieName, TFunction<void(int)> Completed)
{
...
TSharedPtr<IBrowserCoreCookieManager> FBrowserCoreSingleton::GetCookieManager(TOptional<FString> ContextId) const
{
	if (!ContextId.IsSet())
	{
	...
	if (ExistingContext)
	{
		...
		return FCefBrowserCoreCookieManagerFactory::Create((*ExistingContext)->GetDefaultCookieManager(nullptr));
	}
#endif
...
bool FBrowserCoreSingleton::RegisterContext(const FBrowserContextSettings& Settings)
{
#if WITH_CEF3
...
bool FBrowserCoreSingleton::UnregisterContext(const FString& ContextId)
{
#if WITH_CEF3
