...
#include "BrowserCoreHandler.h"
#include "BrowserCoreModule.h"
#include "BrowserCoreWindow.h"
#include "BrowserCoreClosureTask.h"
...
#define LOCTEXT_NAMESPACE "BrowserCoreHandler"
...
FBrowserCoreHandler::FBrowserCoreHandler(bool InUseTransparency)
: bUseTransparency(InUseTransparency)
{ }

void FBrowserCoreHandler::OnTitleChange(CefRefPtr<CefBrowser> Browser, const CefString& Title)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
void FBrowserCoreHandler::OnAddressChange(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, const CefString& Url)
{
	if (Frame->IsMain())
	{
		TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
bool FBrowserCoreHandler::OnTooltip(CefRefPtr<CefBrowser> Browser, CefString& Text)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
	{
...
void FBrowserCoreHandler::OnAfterCreated(CefRefPtr<CefBrowser> Browser)
{
	if(Browser->IsPopup())
	{
		TSharedPtr<FBrowserCoreWindow> BrowserWindowParent = ParentHandler.get() ? ParentHandler->BrowserWindowPtr.Pin() : nullptr;
		if(BrowserWindowParent.IsValid() && ParentHandler->OnCreateWindow().IsBound())
		{
			TSharedPtr<FBrowserCoreWindowInfo> NewBrowserWindowInfo = MakeShareable(new FBrowserCoreWindowInfo(Browser, this));
			TSharedPtr<IBrowserCoreWindow> NewBrowserWindow = IBrowserCoreModule::Get().GetSingleton()->CreateBrowserWindow( 
				BrowserWindowParent,
...

			{
				...
				TSharedPtr<FBrowserCoreWindow> HandlerSpecificBrowserWindow = StaticCastSharedPtr<FBrowserCoreWindow>(NewBrowserWindow);
				BrowserWindowPtr = HandlerSpecificBrowserWindow;
			}

			// Request a UI window for the browser.  If it is not created we do some cleanup.
			bool bUIWindowCreated = ParentHandler->OnCreateWindow().Execute(TWeakPtr<IBrowserCoreWindow>(NewBrowserWindow), TWeakPtr<IBrowserCorePopupFeatures>(BrowserPopupFeatures));
			if(!bUIWindowCreated)
			{
			...
			{
				checkf(!NewBrowserWindow.IsUnique(), TEXT("Handler indicated that new window UI was created, but failed to save the new BrowserCoreWindow instance."));
			}
		}
		else
		{
...
bool FBrowserCoreHandler::DoClose(CefRefPtr<CefBrowser> Browser)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if(BrowserWindow.IsValid())
	{
...
void FBrowserCoreHandler::OnBeforeClose(CefRefPtr<CefBrowser> Browser)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
	{
...
bool FBrowserCoreHandler::OnBeforePopup( CefRefPtr<CefBrowser> Browser,
	...
	bool* OutNoJavascriptAccess )
{
	...
	if((OnBeforePopup().IsBound() && OnBeforePopup().Execute(URL, FrameName)) || !OnCreateWindow().IsBound())
	{
		...
		{
		TSharedPtr<FBrowserCorePopupFeatures> NewBrowserPopupFeatures = MakeShareable(new FBrowserCorePopupFeatures(PopupFeatures));
		...
		CefRefPtr<FBrowserCoreHandler> NewHandler(new FBrowserCoreHandler(shouldUseTransparency));
...
bool FBrowserCoreHandler::OnCertificateError(CefRefPtr<CefBrowser> Browser,
...
void FBrowserCoreHandler::OnLoadError(CefRefPtr<CefBrowser> Browser,
	CefRefPtr<CefFrame> Frame,
	...
	if (Frame->IsMain())
	{
		...
		if (BrowserWindow.IsValid())
		{
			...
			if (BrowserWindow->IsShowingErrorMessages())
			{
				...
				FText ErrorMsg = FText::Format(LOCTEXT("BrowserCoreLoadError", "Failed to load URL {FailedUrl} with error {ErrorText} ({ErrorCode})."), Args);
...
void FBrowserCoreHandler::OnLoadStart(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame)
{
}

void FBrowserCoreHandler::OnLoadingStateChange(CefRefPtr<CefBrowser> Browser, bool bIsLoading, bool bCanGoBack, bool bCanGoForward)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
bool FBrowserCoreHandler::GetRootScreenRect(CefRefPtr<CefBrowser> Browser, CefRect& Rect)
{
...
bool FBrowserCoreHandler::GetViewRect(CefRefPtr<CefBrowser> Browser, CefRect& Rect)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
void FBrowserCoreHandler::OnPaint(CefRefPtr<CefBrowser> Browser,
...
void FBrowserCoreHandler::OnCursorChange(CefRefPtr<CefBrowser> Browser, CefCursorHandle Cursor, CefRenderHandler::CursorType Type, const CefCursorInfo& CustomCursorInfo)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
void FBrowserCoreHandler::OnPopupShow(CefRefPtr<CefBrowser> Browser, bool bShow)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
void FBrowserCoreHandler::OnPopupSize(CefRefPtr<CefBrowser> Browser, const CefRect& Rect)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
CefRequestHandler::ReturnValue FBrowserCoreHandler::OnBeforeResourceLoad(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefRefPtr<CefRequest> Request, CefRefPtr<CefRequestCallback> Callback)
{
	// Current thread is IO thread. We need to invoke BrowserWindow->GetResourceContent on the UI (aka Game) thread:
	CefPostTask(TID_UI, new FBrowserCoreClosureTask(this, [=]()
	{
		const FString LanguageHeaderText(TEXT("Accept-Language"));
		const FString LocaleCode = FBrowserCoreSingleton::GetCurrentLocaleCode();
...
void FBrowserCoreHandler::OnRenderProcessTerminated(CefRefPtr<CefBrowser> Browser, TerminationStatus Status)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
	{
...
bool FBrowserCoreHandler::OnBeforeBrowse(CefRefPtr<CefBrowser> Browser,
	CefRefPtr<CefFrame> Frame,
...
CefRefPtr<CefResourceHandler> FBrowserCoreHandler::GetResourceHandler( CefRefPtr<CefBrowser> Browser, CefRefPtr< CefFrame > Frame, CefRefPtr< CefRequest > Request )
{
	...
	if (Request->GetMethod() == *CustomContentMethod)
	{
		...
		if (ContentOverride != HeaderMap.end())
		{
			...
			return new FBrowserCoreByteResource(Convert.c_str(), Convert.length());
		}
	}
	return nullptr;
}

void FBrowserCoreHandler::SetBrowserWindow(TSharedPtr<FBrowserCoreWindow> InBrowserWindow)
{
	BrowserWindowPtr = InBrowserWindow;
}

bool FBrowserCoreHandler::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser,
...
bool FBrowserCoreHandler::ShowDevTools(const CefRefPtr<CefBrowser>& Browser)
{
	CefPoint Point;
...
bool FBrowserCoreHandler::OnKeyEvent(CefRefPtr<CefBrowser> Browser,
	...
	CefEventHandle OsEvent)
{
	...
	if( (Event.type == KEYEVENT_RAWKEYDOWN || Event.type == KEYEVENT_KEYDOWN) &&
		...
		IBrowserCoreModule::Get().GetSingleton()->IsDevToolsShortcutEnabled()
	  )
	{
...
bool FBrowserCoreHandler::OnJSDialog(CefRefPtr<CefBrowser> Browser, const CefString& OriginUrl, const CefString& AcceptLang, JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	bool Retval = false;
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
...
bool FBrowserCoreHandler::OnBeforeUnloadDialog(CefRefPtr<CefBrowser> Browser, const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	bool Retval = false;
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
...
void FBrowserCoreHandler::OnResetDialogState(CefRefPtr<CefBrowser> Browser)
{
	TSharedPtr<FBrowserCoreWindow> BrowserWindow = BrowserWindowPtr.Pin();
	if (BrowserWindow.IsValid())
	{
