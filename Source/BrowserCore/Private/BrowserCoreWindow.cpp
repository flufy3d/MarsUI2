...
namespace {
	...
	class FBrowserCoreClosureVisitor
		: public CefStringVisitor
	{
	public:
		FBrowserCoreClosureVisitor(TFunction<void (const FString&)> InClosure)
			: Closure(InClosure)
		{ }
	...
	private:
		...
		IMPLEMENT_REFCOUNTING(FBrowserCoreClosureVisitor);
	};
}
...
FBrowserCoreWindow::FBrowserCoreWindow(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FBrowserCoreHandler> InHandler, FString InUrl, TOptional<FString> InContentsToLoad, bool InShowErrorMessage, bool InThumbMouseButtonNavigation, bool InUseTransparency)
	: DocumentState(EBrowserCoreDocumentState::NoDocument)
	, InternalCefBrowser(InBrowser)
	, BrowserCoreHandler(InHandler)
	, CurrentUrl(InUrl)
...
{
	BrowserCoreHandler->OnCreateWindow().Unbind();
	BrowserCoreHandler->OnBeforePopup().Unbind();
	CloseBrowser(true);
...
FSlateShaderResource* FBrowserCoreWindow::GetTexture(bool bIsPopup)
{
...
void FBrowserCoreWindow::GetSource(TFunction<void (const FString&)> Callback) const
{
	if (IsValid())
	{
		InternalCefBrowser->GetMainFrame()->GetSource(new FBrowserCoreClosureVisitor(Callback));
	}
	else
	{
		Callback(FString());
...
void FBrowserCoreWindow::PopulateCefKeyEvent(const FKeyEvent& InKeyEvent, CefKeyEvent& OutKeyEvent)
{
#if PLATFORM_MAC
...
bool FBrowserCoreWindow::OnUnhandledKeyEvent(const CefKeyEvent& CefEvent)
{
...
bool FBrowserCoreWindow::OnJSDialog(CefJSDialogHandler::JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	bool Retval = false;
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IBrowserCoreDialog> Dialog(new FBrowserCoreDialog(DialogType, MessageText, DefaultPromptText, Callback));
		EBrowserCoreDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IBrowserCoreDialog>(Dialog));
		switch (EventResponse)
		{
		case EBrowserCoreDialogEventResponse::Handled:
			Retval = true;
			break;
		case EBrowserCoreDialogEventResponse::Continue:
			...
			break;
		case EBrowserCoreDialogEventResponse::Ignore:
			OutSuppressMessage = true;
			Retval = false;
			break;
		case EBrowserCoreDialogEventResponse::Unhandled:
		default:
			Retval = false;
			break;
		}
...
bool FBrowserCoreWindow::OnBeforeUnloadDialog(const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	bool Retval = false;
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IBrowserCoreDialog> Dialog(new FBrowserCoreDialog(MessageText, IsReload, Callback));
		EBrowserCoreDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IBrowserCoreDialog>(Dialog));
		switch (EventResponse)
		{
		case EBrowserCoreDialogEventResponse::Handled:
...
void FBrowserCoreWindow::OnResetDialogState()
{
	OnDismissAllDialogs().ExecuteIfBound();
}

void FBrowserCoreWindow::OnRenderProcessTerminated(CefRequestHandler::TerminationStatus Status)
{
...
void FBrowserCoreWindow::CloseBrowser(bool bForce)
{
	if (IsValid())
	{
		...
		CefPostTask(TID_UI, new FBrowserCoreClosureTask(nullptr, [=]()
		{
...
CefRefPtr<CefBrowser> FBrowserCoreWindow::GetCefBrowser()
{
	return InternalCefBrowser;
}

void FBrowserCoreWindow::SetTitle(const CefString& InTitle)
{
	Title = InTitle.ToWString().c_str();
	TitleChangedEvent.Broadcast(Title);
}

void FBrowserCoreWindow::SetUrl(const CefString& Url)
{
	CurrentUrl = Url.ToWString().c_str();
	OnUrlChanged().Broadcast(CurrentUrl);
}

void FBrowserCoreWindow::SetToolTip(const CefString& CefToolTip)
{
...
bool FBrowserCoreWindow::GetViewRect(CefRect& Rect)
{
...
void FBrowserCoreWindow::NotifyDocumentError(int InErrorCode)
{
	ErrorCode = InErrorCode;
	DocumentState = EBrowserCoreDocumentState::Error;
	DocumentStateChangedEvent.Broadcast(DocumentState);
}

void FBrowserCoreWindow::NotifyDocumentLoadingStateChange(bool IsLoading)
{
	if (! IsLoading)
	{
		bIsInitialized = true;
	...
	if (IsLoading || DocumentState != EBrowserCoreDocumentState::Error)
	{
		ErrorCode = 0;
		DocumentState = IsLoading
			? EBrowserCoreDocumentState::Loading
			: EBrowserCoreDocumentState::Completed;
		DocumentStateChangedEvent.Broadcast(DocumentState);
	}

}

void FBrowserCoreWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
...
void FBrowserCoreWindow::UpdateVideoBuffering()
{
...
void FBrowserCoreWindow::OnCursorChange(CefCursorHandle CefCursor, CefRenderHandler::CursorType Type, const CefCursorInfo& CustomCursorInfo)
{
	switch (Type) {
...
bool FBrowserCoreWindow::OnBeforeBrowse( CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefRefPtr<CefRequest> Request, bool bIsRedirect )
{
...
TOptional<FString> FBrowserCoreWindow::GetResourceContent( CefRefPtr< CefFrame > Frame, CefRefPtr< CefRequest > Request)
{
...
int32 FBrowserCoreWindow::GetCefKeyboardModifiers(const FKeyEvent& KeyEvent)
{
...
int32 FBrowserCoreWindow::GetCefMouseModifiers(const FPointerEvent& InMouseEvent)
{
...
CefMouseEvent FBrowserCoreWindow::GetCefMouseEvent(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	CefMouseEvent Event;
...
int32 FBrowserCoreWindow::GetCefInputModifiers(const FInputEvent& InputEvent)
{
	int32 Modifiers = 0;
...
void FBrowserCoreWindow::CheckTickActivity()
{
...
void FBrowserCoreWindow::SetIsHidden(bool bValue)
{
	if( bIsHidden == bValue )
	{
		return;
...
CefRefPtr<CefDictionaryValue> FBrowserCoreWindow::GetProcessInfo()
{
...
bool FBrowserCoreWindow::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
	return Scripting->OnProcessMessageReceived(Browser, SourceProcess, Message);
}

void FBrowserCoreWindow::BindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	Scripting->BindUObject(Name, Object, bIsPermanent);
}

void FBrowserCoreWindow::UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	Scripting->UnbindUObject(Name, Object, bIsPermanent);
}


void FBrowserCoreWindow::OnBrowserClosing()
{
	bIsClosing = true;
}

void FBrowserCoreWindow::OnBrowserClosed()
{
	if(OnCloseWindow().IsBound())
	{
		OnCloseWindow().Execute(TWeakPtr<IBrowserCoreWindow>(SharedThis(this)));
	}
...
void FBrowserCoreWindow::SetPopupMenuPosition(CefRect CefPopupSize)
{
...
void FBrowserCoreWindow:: ShowPopupMenu(bool bShow)
{
	if (bShow)
	{
