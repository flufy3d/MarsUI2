...
bool FWebJSScripting::HandleExecuteUObjectMethodMessage(CefRefPtr<CefListValue> MessageArguments)
{
		...
		for ( TFieldIterator<UProperty> It(Function); It; ++It )
		{
			...
			if (Param->PropertyFlags & CPF_Parm)
			{
				...
				else
				{
					...
					if (StructProperty && StructProperty->Struct->IsChildOf(FCoreJSResponse::StaticStruct()))
					{
						PromiseParam = Param;
					}
					else
	...
	if (PromiseParam)
	{
		FCoreJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FCoreJSResponse>(Params.GetData());
		if (PromisePtr)
		{
			*PromisePtr = FCoreJSResponse(SharedThis(this), ResultCallbackId);
		}
	}
