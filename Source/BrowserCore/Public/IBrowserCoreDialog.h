...
enum class EBrowserCoreDialogType
{
 ...
 */
enum class EBrowserCoreDialogEventResponse
{
	...
	Ignore,
	/** The event handler will take care of showing the dialog. It must call IBrowserCoreDialog::Continue once it has been dismissed. */
	Handled,
};
 ...
 */
class IBrowserCoreDialog
{
public:

	virtual ~IBrowserCoreDialog()
	{}
	 ...
	 */
	virtual EBrowserCoreDialogType GetType() = 0;

	/**
	 * Tell the browser to continue with the result of the dialog.
	 * If this method is used, the original event handler must return EBrowserCoreDialogEventResponse::Handled.
	 *
