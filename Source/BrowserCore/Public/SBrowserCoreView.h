...
enum class EBrowserCoreDocumentState;
class IBrowserCoreWindow;
class FBrowserCoreViewport;
class UObject;
class IBrowserCorePopupFeatures;
class IBrowserCoreAdapter;
struct FWebNavigationRequest;

DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnBeforePopupDelegate, FString, FString);
DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnCreateWindowDelegate, const TWeakPtr<IBrowserCoreWindow>&, const TWeakPtr<IBrowserCorePopupFeatures>&);
DECLARE_DELEGATE_RetVal_OneParam(bool, FOnCloseWindowDelegate, const TWeakPtr<IBrowserCoreWindow>&);

#if WITH_CEF3
typedef SViewport SBrowserCoreWidget;
#else
typedef SWidget SBrowserCoreWidget;
#endif

class BROWSERCORE_API SBrowserCoreView
...
public:
	...
	DECLARE_DELEGATE_RetVal_OneParam(EBrowserCoreDialogEventResponse, FOnShowDialog, const TWeakPtr<IBrowserCoreDialog>&);

	SLATE_BEGIN_ARGS(SBrowserCoreView)
	 ...
	 */
	void Construct(const FArguments& InArgs, const TSharedPtr<IBrowserCoreWindow>& InBrowserCoreWindow = nullptr);
	...
	*/
	...
	void BindAdapter(const TSharedRef<IBrowserCoreAdapter>& Adapter);

	void UnbindAdapter(const TSharedRef<IBrowserCoreAdapter>& Adapter);
...
private:
	...
	void HandleBrowserWindowDocumentStateChanged(EBrowserCoreDocumentState NewState);
	 ...
	 */
	bool HandleCreateWindow(const TWeakPtr<IBrowserCoreWindow>& NewBrowserWindow, const TWeakPtr<IBrowserCorePopupFeatures>& PopupFeatures);
	 ...
	 */
	bool HandleCloseWindow(const TWeakPtr<IBrowserCoreWindow>& BrowserWindow);

	/** Callback for showing dialogs to the user */
	EBrowserCoreDialogEventResponse HandleShowDialog(const TWeakPtr<IBrowserCoreDialog>& DialogParams);
...
private:
	...
	TSharedPtr<IBrowserCoreWindow> BrowserWindow;
	...
	TSharedPtr<FBrowserCoreViewport> BrowserViewport;
	/** Viewport interface for rendering popup menus. */
	TSharedPtr<FBrowserCoreViewport>	MenuViewport;
	/** The implementation dependent widget that renders the browser contents. */
	TSharedPtr<SBrowserCoreWidget> BrowserWidget;

	TArray<TSharedRef<IBrowserCoreAdapter>> Adapters;
