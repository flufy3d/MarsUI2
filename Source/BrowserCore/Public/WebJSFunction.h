...
struct BROWSERCORE_API FWebJSParam
{
...
USTRUCT()
struct BROWSERCORE_API FCoreJSCallbackBase
{
...
protected:
	FCoreJSCallbackBase(TSharedPtr<FWebJSScripting> InScripting, const FGuid& InCallbackId)
 ...
 * FCoreJSFunction objects represent a JS function and allow calling them from native code.
 * FCoreJSFunction objects can also be added to delegates and events using the Bind/AddLambda method.
 */
USTRUCT()
struct BROWSERCORE_API FCoreJSFunction
	: public FCoreJSCallbackBase
{
	GENERATED_USTRUCT_BODY()
	...
	FCoreJSFunction(TSharedPtr<FWebJSScripting> InScripting, const FGuid& InFunctionId)
		: FCoreJSCallbackBase(InScripting, InFunctionId)
	{}
 ...
 *  UFUNCTIONs taking a FCoreJSResponse will get it passed in automatically when called from a web browser.
 *  Pass a result or error back by invoking Success or Failure on the object.
 *  UFunctions accepting a FCoreJSResponse should have a void return type, as any value returned from the function will be ignored.
 ...
 */
USTRUCT()
struct BROWSERCORE_API FCoreJSResponse
	: public FCoreJSCallbackBase
{
	GENERATED_USTRUCT_BODY()
	...
	FCoreJSResponse(TSharedPtr<FWebJSScripting> InScripting, const FGuid& InCallbackId)
		: FCoreJSCallbackBase(InScripting, InCallbackId)
	{}
